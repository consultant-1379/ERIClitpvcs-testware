...
public class LITPvcsTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPvcsTestRunner.class);
    ...
    @TestId(id = "CXP9031134-1", title = "Upgrade ERIClitpvcs rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpvcsRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("vcs");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031134-2", title = "Run python test cases for ERIClitpvcs package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpvcsTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpvcsTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031134-3", title = "Parse xml outputs from python tests")
